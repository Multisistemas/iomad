...
require_once ($CFG->dirroot . '/local/paradisolms/lib.php');
...
if(isset($_GET['user']) && isset($_GET['token']) && isset($_GET['ts']) && isset($_GET['email']))
{
    ...
    if($localToken == $token)
    {
        ...
        if($current_time <= $timestamp + (30 * 60))
        {
            ...
            // the user exists so lets process cohorts and login
            if(! empty($localUser))
            {
                // get user company
            ...
            else
            {
                ...
                if($created && $newUser)
                {
                    ...
                    try
                    {
                        ...
                        auth_token_assign_role_to_admin($USER->id);
                        
                        if($company != '')
                        {
                            $companyid = auth_token_create_company($company, $city, $country);
                            
                            if($companyid > 0)
                            {
                                auth_token_assign_admin_to_company($USER->id, $companyid);
                                
                                if($courses != '')
                                {
                                    $arrCourses = get_courses_from_param($courses);
                            ...
                            {
                                paradisolms_log("Cannot create company. Name: $company, City: $city, Country: $country");
                            }
                        }
                    ...
                    {
                        paradisolms_log('Cannot do SSO. Unexpected error', $e);
                        $transaction->rollback($e);
                    }
                    
                    auth_token_process_cohorts($USER->id, $cohorts);
                    auth_token_process_courses($USER->id, $courses, $companyid);
                    do_login($USER);
                }
                else
                {
                    paradisolms_log('Cannot do SSO. Unable to create the new user');
        ...
        {
            paradisolms_log('Cannot do SSO. Timeout');
    ...
    {
        paradisolms_log('Cannot do SSO. Token does not macth');
    }
}
else
{
    paradisolms_log('Cannot do SSO. Missing parametters');
