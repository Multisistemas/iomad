...
function do_login($localUser, $urltogo = null)
{
    ...
    if($user === false)
    {
        paradisolms_log("Unable to login user with username $username", array(
            'localuser' => $localuser 
        ));
    ...
    {
        paradisolms_log("User authenticated with error", array(
            'error' => $err,
            'username' => $username 
        ));
...
function auth_token_process_cohorts($userid, $cohorts)
{
    ...
    if($cohorts != '')
    {
        ...
        foreach($arrCohorts as $idnumnber)
        {
            ...
            {
                paradisolms_log("Unable to add to cohort. Cohort not found by idnumber $idnumnber");
            }
        }
    }
    else
    {
        paradisolms_log('Unable to process cohorts. $cohorts variable is empty');
    }
}
...
function auth_token_process_courses($userid, $courses, $companyid)
{
    ...
    if($courses != '')
    {
        ...
        foreach($arrCourses as $idnumnber)
        {
            ...
            {
                paradisolms_log("Unable to enroll to course. Course not found by idnumber $idnumnber");
            }
        }
    }
    else
    {
        paradisolms_log('Unable to process courses. $courses variable is empty');
    }
}
...
function auth_token_create_company($name, $city, $country)
{
    ...
    */
    ...
    {
        $companyid = 0;
        paradisolms_log("Unable to create company. Already exist a company with shortname $companyshortname");
    }
    else
    {
        ...
        if(! $companyid)
        {
            paradisolms_log("Unable to create company with shortname $companyshortname", array(
...
function auth_token_assign_admin_to_company($userid, $companyid)
{
    ...
    if(! $cuid)
    {
        paradisolms_log("Unable to associate user $userid as manager for company $companyid");
        $cuid = 0;
    }
...
function auth_token_assign_role_to_admin($userid)
{
    ...
    if(! $raid)
    {
        paradisolms_log("Unable to assign role {$rarecord->roleid} to user $userid");
        $raid = 0;
    }
...
function auth_token_enrol_user_to_course($userid, $courseid)
{
    ...
    if($studentrole)
    {
        ...
        else
        {
            ...
            paradisolms_log("Unable to assign get manual entol instance", $info);
        }
    }
    else
    {
        ...
        paradisolms_log("Unable to enrol user to course. Student role not found.", $info);
    }
}
